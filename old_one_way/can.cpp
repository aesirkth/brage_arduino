...
CircularBuffer<canRec, MAX_LENGTH> rxBuf;   // LoRa -> CAN
CircularBuffer<canRec, MAX_LENGTH> txBuf;   // CAN  -> LoRa
...
void initCan() {
  ...
  Serial.printf("[CAN] Init returned (%d), ErrorCode=0x%08lx\n", ret, hfdcan1.ErrorCode);
  if (ret != HAL_OK) {
    Serial.println("[CAN] Init FAILED");
    return;
  }

  // configure a match all filter
  ...
  Serial.printf("[CAN] Filter config returned (%d), ErrorCode=0x%08lx\n", ret, hfdcan1.ErrorCode);
  if (ret != HAL_OK) {
  ...
  Serial.printf("[CAN] Start returned (%d), ErrorCode=0x%08lx\n", ret, hfdcan1.ErrorCode);
}

// Check for received CAN messages and push them to txBuf
void pollCanRx() {
  // Check FIFO 0 fill level
...
void processCanTx() {
  ...
  while (!rxBuf.isEmpty() && HAL_FDCAN_GetTxFifoFreeLevel(&hfdcan1) > 0) {
    ...
    if (HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &txHeader, rec.data) == HAL_OK) {
      ...
      Serial.printf("[CAN] AddMessageToTxFifoQ failed, Status=0x%lx, ErrorCode=0x%lx\n",
