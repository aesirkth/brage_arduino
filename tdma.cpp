...
#define TDMA_LOGF(...)                                                         \
  do {                                                                         \
    if (TDMA_ENABLE_DEBUG) {                                                   \
      Serial.printf(__VA_ARGS__);                                              \
    }                                                                          \
  } while (0)
#define TDMA_LOGLN(msg)                                                        \
  do {                                                                         \
    if (TDMA_ENABLE_DEBUG) {                                                   \
      Serial.println(msg);                                                     \
    }                                                                          \
...
static void tdmaEnterSlot(SlotId next_slot) {
  ...
  case UPLINK:
    ...
    if (state.role == TDMA_FOLLOWER) {
      ...
      TDMA_LOGF(
          "[TDMA] UPLINK entry: synced=%d empty=%d tx=%d frame=%u slot=%u\n",
          (int)state.synced, (int)!hasData ? 1 : 0, (int)canTx,
          (unsigned int)state.frameSeq, (unsigned int)state.currentSlot);
      if (canTx) {
        delay(1); // wait before transmit
        tdmaTransmit();
      } else {
        startRx();
      }
...
void tdmaProcessRx(const uint8_t *buf, size_t len, uint32_t rx_time) {
  ...
  // Master receiving UPLINK has no header, follower receiving DOWNLINK has
  // header
  ...
  if (parseHeader) {
    ...
    if (slot_id_raw != GUARD && slot_id_raw != UPLINK &&
        slot_id_raw != DOWNLINK) {
      return;
    }
    ...
    TDMA_LOGF("[TDMA] RX header: slot = %d, frame = %d, epoch = %lld, "
              "num_records = %d\n",
              slot_id_raw, frame_seq, epoch_us, num_records);

    // Follower synchronization from DOWNLINK header
    if ((SlotId)slot_id_raw == DOWNLINK) {
      // Estimate when the master started the frame by backing out guard + half
      // slot
  ...
  // Extract CAN records using while loop - more robust than relying on
  // num_records
  uint8_t rec_count = 0;
  while (offset + sizeof(canRec) <= len) {
    ...
    TDMA_LOGF("  RX rec %u: id=0x%lx dlc=%u\n", (unsigned int)rec_count,
              (unsigned long)rec.id, (unsigned int)rec.dlc);
    rec_count++;
  }
}
...
static void tdmaTransmit() {
  ...
  if (state.role == TDMA_MASTER) {
    ...
    TDMA_LOGF("[TDMA] TX hdr slot=%u frame=%u records=%u\n",
              (unsigned int)header.slot_id, (unsigned int)header.frame_seq,
    ...
    TDMA_LOGF("[TDMA] TX frame=%u slot=%u records=%u (currentSlot=%u)\n",
              (unsigned int)header.frame_seq, (unsigned int)header.slot_id,
              (unsigned int)header.num_records,
...
bool tdmaIsSynced() { return state.synced; }
